...
| 8   | Reconciliação Tax-BR × PortfolioCalculator (item 18 do diagnóstico) | **PASS + 1 caracterização**  | `case-08-quantity-cost-basis-reconciliation.spec.ts` (1 teste): compra dupla de BOVA11.SA (10@130 + 5@140, taxas 5+3) reconciliada entre `EquityTaxCalculatorService` e `PortfolioCalculator`. Quantidade bate exatamente (15=15). Custo NÃO bate em valor bruto por desenho: `PortfolioCalculator.averagePrice` é livre de taxa de corretagem (133,33), `EquityTaxCalculatorService.averagePriceBrl` inclui a taxa no custo de aquisição (133,87 — exigência da Receita Federal). Invariante validada: `quantity × averagePrice + fee` (motor central) reconcilia exatamente com `costBasisBrl` (Tax-BR): 2000 + 8 = 2008 nos dois lados | Confirma que os dois motores duplicados (ver item 18) NÃO divergem silenciosamente — a única diferença é o tratamento de taxa, já esperado e agora coberto por guarda de regressão                                         | Nenhuma; reavaliar unificação dos motores só se este teste quebrar no futuro                                                                  |
